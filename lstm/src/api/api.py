...
def predict():
    ...
    x_data_norm = normalize_data(x_data, feature_min_value, feature_max_value).reshape(-1)
    ...
    y_actual = []
    predictions_norm = []
    for i in range(len(y_train) - prediction_days_amount - prediction_offset, len(y_train) - prediction_offset):
    ...
    predicted_output = denormalize_data(np.array(predictions_norm), feature_min_value, feature_max_value).reshape(-1)

...
class LSTMLayer:
    def __init__(
        self, input_size: int, hidden_size: int, dropout_rate: float = 0.0
    ) -> None:
        ...
        self.forget_gate_weights = (
            np.random.randn(hidden_size, input_size + hidden_size) * 0.01
        )
    ...
    def forward(
        ...
        updated_cell_state = (
            forget_gate_output * previous_cell_state
            + input_gate_output * candidate_cell_state
        )
        updated_hidden_state = output_gate_output * tanh(updated_cell_state)

        if self.dropout_rate > 0 and training:
            dropout_mask = (
                np.random.rand(*updated_hidden_state.shape) > self.dropout_rate
            ).astype(float)
    ...
    def backward(
        ...
        for t in reversed(range(len(self.cache))):
            ...
            cell_gradient_t = cell_gradient + hidden_gradient * output_output * (
                1 - tanh(current_cell) ** 2
            )
            cell_candidate_gradient = cell_gradient_t * input_output
            cell_candidate_raw_gradient = cell_candidate_gradient * (
                1 - candidate_cell**2
            )
            ...
            gradient_forget_gate_weights += np.dot(
                forget_raw_gradient, concatenated_input.T
            )
            gradient_forget_gate_bias += forget_raw_gradient
            gradient_input_gate_weights += np.dot(
                input_raw_gradient, concatenated_input.T
            )
            gradient_input_gate_bias += input_raw_gradient
            gradient_output_gate_weights += np.dot(
                output_raw_gradient, concatenated_input.T
            )
            gradient_output_gate_bias += output_raw_gradient
            gradient_cell_state_weights += np.dot(
                cell_candidate_raw_gradient, concatenated_input.T
            )

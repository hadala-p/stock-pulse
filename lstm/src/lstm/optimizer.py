...
class Adam:
    ...
    def update(
        self, params: List[NDArray[np.float64]], grads: List[NDArray[np.float64]]
    ) -> None:
        ...
        for i, (param, grad) in enumerate(zip(params, grads)):
            self.first_moment_estimate[i] = (
                self.beta1 * self.first_moment_estimate[i] + (1 - self.beta1) * grad
            )
            self.second_moment_estimate[i] = self.beta2 * self.second_moment_estimate[
                i
            ] + (1 - self.beta2) * (grad**2)

            first_moment_corrected = self.first_moment_estimate[i] / (
                1 - self.beta1**self.iteration
            )
            second_moment_corrected = self.second_moment_estimate[i] / (

...
class LSTMModel:
    ...
    def forward(
        self, x: NDArray[np.float64], training=False
    ) -> Tuple[NDArray[np.float64], list[NDArray[np.float64]]]:
        hidden_states = [
            np.zeros((hidden_size, 1)) for hidden_size in self.hidden_sizes
        ]
    ...
    def train(self, x_train, y_train, epochs=100, new_stock_data=False):
        for epoch in range(epochs):
            ...
            for index, i in enumerate(random_data_indexes, start=1):
                ...
                predicted_output, hidden_states = self.forward(
                    input_sequence, training=True
                )
                ...
                output_gradient = (
                    2 * (predicted_output - true_output) / true_output.size
                )
                ...
                for i in reversed(range(len(self.lstm_layers))):
                    ...
                    lstm_grad = lstm_layer.backward(
                        next_hidden_gradient, next_cell_gradient
                    )
                ...
                for layer, layer_grads in zip(
                    self.lstm_layers, reversed(lstm_gradients)
                ):
                    params.extend(
                        [
                            layer.forget_gate_weights,
                            layer.forget_gate_bias,
                            layer.input_gate_weights,
                            layer.input_gate_bias,
                            layer.output_gate_weights,
                            layer.output_gate_bias,
                            layer.cell_state_weights,
                            layer.cell_state_bias,
                        ]
                    )
            ...
            if mean_loss < self.best_loss or new_stock_data:
                ...
                print(
                    f"Found new best model with loss: {self.best_loss}. Saving to a file"
                )
